...
class Trainer:
    ...
    def __refresh_training_dataset(self, item_embedding):
        ...
        train_dataset = RecWithContrastiveLearningDataset(self.args, user_seq, time_seq, not_aug_users=not_aug_users,
    ...
    def train(self, epoch):
        ...
        if epoch > self.args.augmentation_warm_up_epochs:
            ...
            self.train_dataloader = self.__refresh_training_dataset(self.model.item_embedding)
        self.iteration(epoch, self.train_dataloader)

    def valid(self, epoch, full_sort=False):
        return self.iteration(epoch, self.eval_dataloader, full_sort=full_sort, mode='valid')

    def test(self, epoch, full_sort=False):
        return self.iteration(epoch, self.test_dataloader, full_sort=full_sort, mode='test')
...
class CoSeRecTrainer(Trainer):
    ...
    def iteration(self, epoch, dataloader, full_sort=True, mode='train'):
        if mode == 'train':
            self.model.train()
            rec_avg_loss = 0.0
